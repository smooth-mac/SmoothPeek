...
> 작성일: 2026-03-09
> 기준 소스: `Sources/DockPeek/` (App 2개, Core 4개, UI 2개 파일)

---

## 1. 프로젝트 개요

DockPeek는 macOS Dock 아이콘에 마우스를 올렸을 때 Windows 작업 표시줄 스타일의 윈도우 미리보기 패널을 띄워주는 메뉴바 유틸리티 앱이다.
...
- 메뉴는 "DockPeek 실행 중" + 종료만 존재 — 환경설정 진입점 없음.
...
- Developer ID 서명 설정.
- `DockPeek.entitlements` 최종 검토 (샌드박스 OFF 명시, 권한 키 확인).
...
3. `DockPeek.entitlements` 파일 내용 확인 및 배포용 최종 권한 설정.
